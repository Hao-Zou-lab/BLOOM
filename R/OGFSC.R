...
#'
#' @param data The log transformed expression data, genes vs. cells.
#' @param nBins The number of gene bins.
#' @param minBinSize The minimum number of genes contained in one gene bin.
#' @param LR_p The linear regression p-value cutoff. Those bins with LR p-vlaue larger than this cutoff should be abandoned.
#' @param TW_threshold The p-value cutoff to select effective eigenvalues.
#' @param plot_option The option to plot OGFSC results, based on which the users can adjust other parameters.
#'
#' @return OGFSC_idx,The index of OGFSC selected genes.
#' @return idx_output,The index of genes successfully modelled by LR.
#' @return cv2_threshold,The baseline filtering threshold.



OGFSC <- function(data, nBins = 30, minBinSize = 100, LR_p = 0.01,
                    TW_threshold = 0.0001, plot_option = 1, paral_option = 1,
                    CV = 10, maxPLScomp = 30, paralSize = 4,
                    scalingMethod = c('mc', 'pa'), sampelLabels = NULL)
  #(OGFSC_idx, idx_output, cv2_threshold)
{
